...
$("#btnsave").on('click',function() {//立即投标
   if($(this).hasClass("invest-btn")){
	    var loginStatus=$("#loginState").val();
		if("true"==loginStatus){
			tender();  
		}else{
			loginWin('pooldetail-details-'+borrowId+'.html')
		}
   }	
});
...
function tender() {//投标
	...
	var autoInvestState=$("#autoInvestState").val();
	...
	if (transAmt == "") {
		...
		huifuBox();
		return ;
	}else if(autoInvestState!=1){
		AutoTenderBox(borrowId);
		return;
	...
	if (!$("#agre").attr("checked")) {
		var poolType = $("#poolBallType").val();//2表示双月盈，3表示四月盈
		var staHtml = poolType == 2 ? '双月盈' : '四月盈';
		xzalert("请勾选我同意投资成功后自动生成《“"+staHtml+"”服务协议》");
		return;
	}
	var param = {};
...
var contrastLoginWin =null;
function checkLoginDialog(){
	        if(!contrastLoginWin){
				contrastLoginWin = popOut({
					title : '验证登录密码',
					width : '535px',
					height : '400px',
					content : ''
				});
				var content = contrastLoginWin.box.find( '.popout-content' );
				var userMobilePhone=$("#userMobilePhone").val();
				html = '<p class="mt50" style="font-size:16px; text-align:center;">为了您的账户资金安全，本次需要验证您小猪罐子账户的登录密码！</p>'+
						'<div name="box" class="popout-2">'+
						'<div class="popout-input m-t-40">'+
						'<em style="text-indent: 2em; display:block;">账户：<i>'+userMobilePhone+'</i></em>'+
						'</div>'+
						'<div class="popout-input">'+
						'<em style="line-height:40px">登录密码：</em>'+
						'<input type="password" id="checkPassWord" name="pwd" id="pwd" maxlength="22" placeholder="请输入密码" style="width:220px;">'+
						'</div>'+
						'<div class="popout-input">'+
						'<em style="text-indent: 5em; width:80px; display:inline-block;"></em><small id="checkPwdErrorTip" class="hide checkPwdWarn" style="position: initial; line-height:1">密码不能为空</small>'+
						'</div>'+
						'<a style="width:220px; margin-left:198px;" href="javascript:checkPwd();" class="popout-button">立即验证</a>'+
						'</div>';

				content.html(html);
			}else{
				$("#checkPassWord").val("");
				$("#checkPwdErrorTip").attr("class","hide checkPwdWarn");
			}
			contrastLoginWin.show();
}

function checkPwd(){
	var checkPassword=$("#checkPassWord").val();
	var checkPwdErrorTip=$("#checkPwdErrorTip");
	if(!checkPassword || !$.trim(checkPassword)){
		checkPwdErrorTip.attr("class","checkPwdWarn");
		checkPwdErrorTip.html("密码不能为空");
		return;
	}
	checkPwdErrorTip.attr("class","hide checkPwdWarn");
	$.post("verifyPassword",{'pwd':checkPassword},function(bjson){
		//密码错误,您还有1次输入机会！
		//该账户已被锁定,请联系客服!
		if(bjson.result==-1){
			checkPwdErrorTip.attr("class","checkPwdWarn");
			checkPwdErrorTip.html("服务器异常");
		}else if(bjson.result==-3 || (bjson.result==-2 &&　bjson.count==0) ){
			checkPwdErrorTip.attr("class","checkPwdWarn");
			checkPwdErrorTip.html("该账户已被锁定,请联系客服!");
		}else if(bjson.result==-2){
			checkPwdErrorTip.attr("class","checkPwdWarn");
			checkPwdErrorTip.html("密码错误,您还有"+bjson.count+"次输入机会！");
		}else{
			if(contrastLoginWin){
				contrastLoginWin.hide();
			}
			//校验成功
			var   dialogWin=popOut({
                ...
                <a href="pooldetail-details-'+borrowId+'.html" class="btn btn-green">已完成投标</a>\
...
function getTickets( cb ){
	...
	param["paramMap.deadline"] = $("#closePeriod").val();//该产品是否可用加息券
	param["paramMap.isDayThe"] = $("#periodType").val();
	...
	$.post("getCouponList",param,function(data){
		if(data.msg=="nologin"){
			loginWin(""+basePath+"/pooldetail-details-"+borrowId+".html");
		}else{
			cb( data );
		}
     });
...
function newShowContract(){
	var html ='<iframe style="border:none;overflow:auto;width:100%;height:480px;" src="showRiskAgreement"></iframe>';
