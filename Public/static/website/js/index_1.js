...
Banner.prototype = {
	...
	jumpTo : function( index ){
		...
		this.cuIndex = ( this.length + index ) % this.length;
		this.move();
	},
	// 实体类实现,图片切换动画
	move : function(){}
...
$(function(){
	...
	$( '.banner-widget' ).each(function(){
		...
		banner.move = function(){
			...
			list.css( 'z-index' , 1 ).show();
			list.eq( this.cuIndex ).css( 'z-index' , 2 );
			list.eq( this.lastIndex ).css( 'z-index' , 3 ).fadeOut();
			controlChange( this.cuIndex );
		}

		function controlChange( index ){
			control.removeClass( 'active' ).html( '○' );
			control.eq( index ).addClass( 'active' ).html( '●' );
		}
