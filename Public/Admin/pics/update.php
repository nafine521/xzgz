...
if (empty($_FILES) === false) {
	...
	include './include/thumb.class.php';
	$new_pic_path=$file_path;

	$_thumb = new ThumbHandler ();
	$_thumb->setSrcImg ( $file_path );//设置图片  资源、宽、高。
	$_thumb->setDstImg ( $new_pic_path );//
	$_thumb->setImgCreateQuality ( 80 );//设置图片的品质
	$_thumb->setMaskPosition ( 4 );//设置位置
  	$_thumb->font_size = 28;
	$_thumb->mask_txt_pct=0;
	$_thumb->font = 10;

	//echo "<a href=\"".$file_url."\" target=\"_blank\">原图[$file_url]</a><br />";
	//echo "文件所在目录 $save_path <p></p>";
